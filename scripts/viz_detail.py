#!/usr/bin/env python3
"""Per-sample pipeline viz — 2x3 grid + HD Map.
...
from assemble_hdmap import _chaikin, _segment_intersection
...
from assemble_hdmap import graph_to_map
...
dl = make_field_dataloader("val", batch_size=6, num_workers=0, limit_samples=6, cache_fields=False)
batch = next(iter(dl))
...
for mw, mh in MAP_SIZES:
    ...
    for i in range(N_PER_SIZE):
        ...
        c7, e7, geoms = merge_parallel_edges(c7, e7, geoms, map_max, angle_deg=20.0, dist_m=30.0)
        # Re-classify after structural changes (merge_parallel may remove nodes)
        nt = classify_nodes(c7, e7, map_max, merge_dist_m=50.0, compressed=True)
        # ── Physically merge nearby junction nodes ────────────────
        c7, e7, nt, geoms = merge_nearby_junctions(c7, e7, nt, geoms, map_max, merge_dist_m=50.0)
        ...
        growth_adj = {i: set() for i in range(len(c6))}
        for a, b in e6:
            growth_adj[int(a)].add(int(b))
            growth_adj[int(b)].add(int(a))
        growth_rc = {}
        for ei_idx in range(len(e6)):
            u, v = int(e6[ei_idx, 0]), int(e6[ei_idx, 1])
        ...
        rc_int = np.ones(len(e7), dtype=np.int64)
        for j in range(len(e7)):
            u, v = int(e7[j, 0]), int(e7[j, 1])
...
for idx, d in enumerate(all_data):
    ...
    for ax, title, c, e, bg, nc, lw in panels:
        ...
        if bg is not None:
            ax.imshow(bg, cmap="gray_r", vmin=0, vmax=1, extent=[0, 1, 0, 1], origin="lower")
        else:
        ...
        if title.startswith("6"):
            ...
            if len(px):
                for u, v in e:
                    ax.plot([px[u, 0], px[v, 0]], [px[u, 1], px[v, 1]], "k-", lw=lw, alpha=0.5)
                ax.scatter(
                    px[:, 0], px[:, 1], c=nc, s=max(2, 12 - len(c) // 50), zorder=3, alpha=0.6
                )
        ax.text(
            0.02,
            0.02,
    ...
    plt.savefig(
        OUT / f"s{idx}_{d['pattern']}_{d['map_w']}x{d['map_h']}.png", dpi=300, bbox_inches="tight"
    )
    plt.close()
