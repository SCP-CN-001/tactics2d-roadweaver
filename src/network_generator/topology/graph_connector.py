...
class EndpointConnector:
    ...
    def __init__(self, map_size_m: float = 2000.0, max_pair_dist_m: float = 600.0):
    ...
    def run(
        ...
        candidates = self._score_candidates(coords, adj, comp_of_nid, comps, comp_eps, road_field)
        ...
        for score, ea, eb in candidates:
            ...
            # Full A* (guiding path)
            path_pts = astar_connect_path(
                anchor_a, anchor_b, road_field, cost_map, W, H, max_steps=10000
            )
        ...
        edge_lengths = [float(np.linalg.norm(coords[int(u)] - coords[int(v)])) for u, v in ei]
        ...
        out_e: set[tuple[int, int]] = {(int(u), int(v)) for u, v in ei}
        closure_pairs: list[tuple[int, int]] = []

        for ea, eb, guiding_path in result_path_groups:
            self._insert_two_phase_path(
                ea,
                eb,
                guiding_path,
                out_c,
        ...
        dense_ce = (
            np.array(closure_pairs, dtype=np.int64).reshape(-1, 2)
            if closure_pairs
            else np.empty((0, 2), dtype=np.int64)
        )

        return self._finalize(
            dense_c, dense_e, dense_ce, N, simplify, merge_distance, self.map_size_m
        )

    # ── Candidate scoring ───────────────────────────────────────────────

    def _score_candidates(
        self, coords, adj, comp_of_nid, comps, comp_eps, road_field
            ...
            for cj in range(ci + 1, len(comp_ids)):
                ...
                for ea in eps_a:
                    ...
                    for eb in eps_b:
                        ...
                        align = (
                            max(0.0, float(np.dot(da, to_u))) + max(0.0, float(np.dot(db, -to_u)))
                        ) / 2.0
                        fs = line_field_support(pa, pb, road_field, n_samples=12, r_px=3)
    ...
    def _simple_astar_insert(
        ...
        path_pts = astar_connect_path(src_coord, tgt_coord, road_field, cost_map, W, H, max_steps)
        if path_pts is None or len(path_pts) < 3:
            return False
    ...
    def _connect_remaining_components(
        self,
        out_c: list,
        out_e: set,
        closure_pairs: list,
        ...
        cur_adj = adjacency_from_edges(
            np.array(list(out_e), dtype=np.int64).reshape(-1, 2), len(out_c)
        )
        ...
        for _ in range(budget):
            ...
            if best_src < 0:
                break
            ok = self._simple_astar_insert(
                best_src, best_tgt, out_c, out_e, native_step, W, H, road_field, cost_map
            )
            if ok:
    ...
    def _finalize(
        ...
        simp_ce = (
            np.array(simp_ce_list, dtype=np.int64).reshape(-1, 2)
            if simp_ce_list
            else np.empty((0, 2), dtype=np.int64)
        )
