...
def graph_to_field(
    coords: np.ndarray,         # (N, 2) — node coordinates, range [0, 1]
    edge_index: np.ndarray,     # (E, 2) — edge indices
    resolution: int = 256,
    sigma_dist: float = 0.02,   # soft distance sigma (in [0,1] space)
    sigma_junction: float = 0.02,
    sigma_endpoint: float = 0.015,
    binary_centerline: bool = True,  # if True, render road_prob as near-binary 0/1
) -> Dict[str, np.ndarray]:
    """
    Render a skeleton graph as a multi-channel raster field.

    Returns dict of (H, W) channels:
        road_prob:      soft distance field (if binary_centerline=False)
                        or near-binary centerline (if binary_centerline=True)
        sin_2theta:     sin(2θ) of road tangent direction (zero off-road)
        ...
        soft_distance:  always-computed soft distance field (auxiliary)
    """
    N = coords.shape[0]
    H = W = resolution
    ...
    # --- Soft distance field (always computed, auxiliary) ---
    ...
    # --- Primary road_prob: either binary centerline or soft distance ---
    if binary_centerline:
        # Near-binary: tiny sigma so only edge pixels ≈ 1
